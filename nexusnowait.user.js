...
;(function () {
  ...
  function isModPage() {
    return MOD_PAGE_PATTERN.test(__cpLocation.pathname)
  }
  ...
  async function getDownloadUrl({ fileId, gameId, isNMM, href }) {
    ...
    if (isNMM && href) {
      ...
      const __cpLocationMatch = res.headers.match(/Location:\s*(nxm:\/\/[^\s]+)/i)
      if (__cpLocationMatch) {
        link = __cpLocationMatch[1]
        return { url: link }
      }
      ...
      if (/ModRequirementsPopUp/.test(href)) {
        ...
        if (downloadHrefMatch) {
          ...
          const __cpLocationMatch2 = res2.headers.match(/Location:\s*(nxm:\/\/[^\s]+)/i)
          if (__cpLocationMatch2) return { url: __cpLocationMatch2[1] }
    ...
    try {
      ...
      const res = await gmRequest(endpoint, {
        method: 'POST',
        data: body,
        headers: { ...headers, Origin: 'https://www.nexusmods.com', Referer: __cpLocation.href }
      })
  ...
  function attachClickInterceptor() {
    async function handleDownload(btn, fileId, isNMM, href) {
      ...
      __cpLocation.assign(url)
    }

    const extractFileId = href => {
      try {
        const url = new URL(href, __cpLocation.href)
    ...
    if (__cpLocation.search.includes('file_id')) {
        ...
        if (modFileDownload?.shadowRoot) {
          ...
          if (slowDownloadBtn) {
            slowDownloadBtn.addEventListener('click', async event => {
              ...
              const { url, error } = await getDownloadUrl({ fileId, gameId: getGameId(), isNMM, href: __cpLocation.href })
              if (error) {
  ...
  function interceptRequirementsTab() {
    document.body.addEventListener(
      ...
      function (event) {
        ...
        __cpLocation.replace(linkHref.replace('tab=requirements', 'tab=files'))
      },
      true
    )
  }
  ...
  function archivedFileHandler() {
    ...
    const base = __cpLocation.origin + __cpLocation.pathname
  ...
  let lastUrl = __cpLocation.href
  const originalPushState = history.pushState
  const originalReplaceState = history.replaceState

  function onNavigate() {
    if (__cpLocation.href !== lastUrl) {
      lastUrl = __cpLocation.href
      main()
    }
  }
